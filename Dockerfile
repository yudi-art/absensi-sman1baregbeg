FROM php:8.4-cli
RUN apt-get update && apt-get install -y git unzip libpng-dev libonig-dev libxml2-dev libzip-dev libpq-dev curl && docker-php-ext-install pdo_mysql mbstring gd zip && apt-get clean && rm -rf /var/lib/apt/lists/*
